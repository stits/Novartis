...
function print_device_list() {
	...
	$query = sprintf("SELECT Devices.Description, Devices.DeviceIP, Locations.Name, " .
			...
			"ConnectionTypes.TypeName, Devices.DeviceName, Devices.Path, Devices.DeviceType, Devices.DeviceID " .
	...
	if(mysql_num_rows($result) > 0) {
		...
		while($row = mysql_fetch_array($result)) {
			...
			switch($row[6]) {
				case "http":
					if($row[9] == "2") {
						$icon = sprintf("<a href=\"launchcam.php?id=%s\" target=\"_blank\"><img src=\"/images/webcam.png\"></a>",
//						$icon = sprintf("<a href=\"javascript:window.open('launchcam.php?id=%s','%s','width=600,height=800')\"><img src=\"/images/webcam.png\"></a>",
							$row[10], $row[0]);
					}
					else {
