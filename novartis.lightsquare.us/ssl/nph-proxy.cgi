...
#     Requires Perl 5.
#
#   Copyright (C) 1996, 1998-2012 by James Marshall, james@jmarshall.com
...
#           the database periodically, with a cron job on Unix/Linux or with
...
use vars qw(
   $DB_DRIVER  $DB_USER  $DB_PASS  $USE_DB_FOR_COOKIES
   %REDIRECTS
   $TEXT_ONLY
...
# [This isn't used yet (as of 2.1.5), but will be.]
...
# Database configuration follows.  Database use is optional.  It's most efficient
#   when this script is running under mod_perl, or as a daemon.
...
#   database, called "cgiproxy", but if your DBA isn't willing to grant the
#   permission to create databases to the CGIProxy user, then you or the DBA will
#   need to create the "cgiproxy" database.
#
# If you are using a database, it must be purged periodically.  In Unix or
#   Linux, do this with a cron job.  In Windows, use the Task Scheduler.
# In Unix or Linux, the command to purge the database is
...
# This is the name of the "database driver" for the database you're using.
#   Currently supported values are "mysql" (note lowercase) and "Oracle".
...
# For now, we're a daemon if GATEWAY_INTERFACE is undefined.
# Not exactly accurate-- is set whenever command-line is used.
$ENV{_DAEMON}= !$ENV{GATEWAY_INTERFACE} ;

# Because of problems assigning STDIN and STDOUT to dup'ed filehandles, we
#   just use these variables for IO, and keep them up to date.
$STDIN= \*STDIN ;
$STDOUT= \*STDOUT ;

# If running as a CGI script, ...
if (!$ENV{_DAEMON}) {
    init unless $HAS_INITED;
    eval { one_run() } ;
    # We'd act on $@, but it does what we need just below anyway.

# else is running as a daemon (more accurately, from the command line).
} else {
    my($cmd, $port_arg) ;
    GetOptions('command|c=s' => \$cmd,
	       'port|p:i' => \$port_arg)  or die "bad options\n" ;
    $port_arg||= 443 ;

    # Start a daemon instead of running as a CGI or mod_perl script.  This is
    #   not complete yet.
    if ($cmd eq 'start-server') {
	...
	if ($LOCK_FH) {
	    ...
	    init ;
	    $pid= spawn_generic_server($HTTPS_LISTEN, $LOCK_FH, \&handle_http_request, 3600, 1) ;  # jsm-- make config option
	}
	print "Port: $port\nPID:  $pid\n" ;
...
sub http_get {
	...
	print S "Connection: close\015\012"  if $HTTP_VERSION eq '1.1' and !$ENV{_DAEMON} ;
...
BEGIN {
	...
	unless ($SSL_CONTEXT) {
	    ...
	    #   or running as a daemon.
	    Net::SSLeay::load_error_strings() if $ENV{MOD_PERL} or $ENV{_DAEMON} ;
	...
	# [This isn't currently used, now that we're not running as a daemon.]
	# jsm-- must make UI for keys....
	if ($is_server) {
	    Net::SSLeay::CTX_use_RSAPrivateKey_file($SSL_CONTEXT, "$main::PROXY_DIR/plain-rsa.pem", &Net::SSLeay::FILETYPE_PEM)
		or Net::SSLeay::die_if_ssl_error("error with private key: $!") ;
	    Net::SSLeay::CTX_use_certificate_file($SSL_CONTEXT, "$main::PROXY_DIR/plain-cert.pem", &Net::SSLeay::FILETYPE_PEM)
	...
	# Following line recurses if an SSL_Handle is in force, like SSL2CLIENT.  :P
...
# jsm-- must add authentication!  Else anyone can use proxy.
...
#$Net::SSLeay::trace= 1 ;
	...
	&daemon_authenticate($ENV{AUTH_TYPE}, $u, $p) ;
...
sub set_ENV_UNCHANGING {
    ...
    # Don't forget non-CGI environment variables
    $ENV_UNCHANGING{_DAEMON}= 1 ;
}


# jsm jsm jsm jsm jsm jsm-- MUST DO THIS!!!
...
# Take the listening socket, the lock filehandle, a code reference, a timeout,
#   and any addition arguments to the code reference as params.  The timeout,
...
sub footer {
    ...
    my $proxified_homepage= full_url('http://www.jmarshall.com/tools/cgiproxy/') ;
    my $download_link= full_url("http://www.jmarshall.com/tools/cgiproxy/releases/cgiproxy.$PROXY_VERSION.tar.gz") ;
...
$safe_URL2= "http://jmarshall.com/bugs/report.cgi?URL=$safe_URL2&version=$PROXY_VERSION" ;
...
the IO::Compress Perl package (several modules), and then this proxy will automatically
support it.  (Note that the IO::Compress package is already included
in Perl 5.9.4 or later.)
EOR
...
#----------------------------------------------------------------------
#  Below is database-related code
#----------------------------------------------------------------------
...
sub require_with_install {
    ...
    if (system('cpan', 'install', $module)) {    # 0 means success
	return undef unless $die_on_failure ;
	&HTMLdie("Couldn't install Perl's $module module.  Try installing it manually.") ;
    }
    eval "require $module" ;
    return 1 unless $@ ;
...
sub create_database {
    ...
    $dbh= $DBH || DBI->connect("dbi:$DB_DRIVER:database=cgiproxy", $DB_USER, $DB_PASS, { AutoCommit => 1 }) ;
    if (!$dbh) {   # jsm-- handle other common error cases!
	# No cgiproxy database yet; try to connect to engine with no database requested.
	$dbh= DBI->connect("dbi:$DB_DRIVER:", $DB_USER, $DB_PASS, { AutoCommit => 1 })
	    or &HTMLdie("Can't connect to database engine: $DBI::errstr") ;
	defined $dbh->do('create database cgiproxy ;')   # jsm-- but what if it's there already?
	    or &HTMLdie("Can't create database 'cgiproxy' (try doing it manually): $DBI::errstr") ;
	# Clobbering old $dbh makes it disconnect.
	$dbh= DBI->connect("dbi:$DB_DRIVER:database=cgiproxy", $DB_USER, $DB_PASS, { AutoCommit => 1 })
	    or &HTMLdie("Can't connect to new 'cgiproxy' database: $DBI::errstr") ;
    }
    # Now $dbh has working handle to cgiproxy database.
